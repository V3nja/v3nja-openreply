...
export default function TesterPage() {
  const [username, setUsername] = useState("bilion_vibez");
  ...
  useEffect(() => {
    void handleSimulate("WAYULOMI is fire! Send link 🔥", "bilion_vibez", true, "COMMENT");
  }, []);
  ...
  return (
    <div className="space-y-8 max-w-6xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Link
          href="/logs"
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 text-zinc-300 hover:text-white font-semibold text-xs transition-all self-start"
        >
          <span>View Live DM Logs</span>
          <span>→</span>
        </Link>
      </div>

      {/* Grid: Simulator Controls & Live Output */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Left: Input Form (Matching Screenshot 2 Complete Your Order styling) */}
          ...
          <div className="glass-card rounded-2xl p-6 space-y-5">
            <div className="flex items-center justify-between pb-3 border-b border-white/[0.06]">
              <h2 className="text-xs font-bold uppercase tracking-wider text-zinc-300 flex items-center gap-2">
                <span>Inbound Trigger Simulator</span>
              </h2>
              <a
                href="https://www.instagram.com/v3nja2.0/"
                target="_blank"
                rel="noreferrer"
                className="text-xs text-amber-400 font-bold bg-amber-500/10 border border-amber-500/20 px-2.5 py-1 rounded-full hover:bg-amber-500/20 transition-colors"
              >
                Target: @v3nja2.0 ↗
              </a>
            </div>
                ...
                {quickKeywords.map((k) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs rounded-xl border transition-all font-bold active:scale-95 ${
                        isSelected
                          ? "bg-gradient-to-r from-amber-500 to-yellow-500 text-black border-amber-400 shadow-md shadow-amber-500/25 scale-[1.03]"
                          : "border-white/10 bg-white/[0.03] hover:border-amber-500/40 text-zinc-300 hover:text-white"
                      }`}
                    >
                      {k.label}
                    </button>
            ...
            >
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    className="w-full pl-9 pr-3.5 py-2.5 text-sm rounded-xl bg-[#18181e] border border-white/10 text-white placeholder:text-zinc-600 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500/40"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2.5 text-sm rounded-xl bg-[#18181e] border border-white/10 text-white placeholder:text-zinc-600 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500/40 resize-none"
                />
              </div>

              {/* V3NJA Studio Gold Action Button */}
              <button
                type="submit"
                onClick={() => void handleSimulate()}
                disabled={loading}
                className="v3nja-gold-button w-full py-3.5 px-4 text-xs uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer"
              >
                {loading ? (
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="glass-card rounded-2xl p-5 space-y-4 shadow-2xl">
            <h3 className="text-xs uppercase font-extrabold tracking-wider text-zinc-400 flex items-center justify-between pb-2 border-b border-white/[0.06]">
            ...
            <div className="p-3.5 rounded-xl bg-[#14141a] border border-white/10 space-y-2">
              ...
              {result?.publicReply && triggerType === "COMMENT" && (
                <div className="ml-3 pl-3 border-l-2 border-amber-500 pt-1 text-xs space-y-0.5">
                  <div className="flex items-center gap-1">
                    <span className="font-bold text-amber-400">@v3nja2.0</span>
            ...
            {/* Direct Message Card */}
            <div className="rounded-2xl bg-[#101015] border border-white/15 overflow-hidden shadow-2xl">
              {/* DM Card Header */}
              <div className="p-3 bg-white/[0.03] border-b border-white/10 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-amber-500 to-yellow-500 flex items-center justify-center font-bold text-[10px] text-black shadow-md">
                    V3
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-0.5">
                  <span className="w-0.5 h-3 bg-amber-400 wave-animation-1 rounded-full" />
                  <span className="w-0.5 h-4 bg-yellow-400 wave-animation-2 rounded-full" />
                  <span className="w-0.5 h-2.5 bg-amber-400 wave-animation-3 rounded-full" />
                </div>
              </div>
                  ...
                  {!isUnlocked && result.isFollowGatedPrompt ? (
                    <div className="space-y-2.5">
                      ...
                      <button
                        ...
                        className="v3nja-gold-button w-full py-2.5 px-3 text-xs uppercase tracking-wider cursor-pointer"
                      >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="p-3 rounded-xl bg-white/[0.03] border border-white/10 text-xs text-zinc-200 whitespace-pre-wrap leading-relaxed">
            ...
            {/* Payment & Streaming Badges (Screenshot 2 exact match) */}
            <div className="pt-2">
              <div className="flex flex-wrap items-center justify-center gap-1.5">
                <span className="px-2.5 py-1 rounded-lg bg-yellow-500/15 border border-yellow-500/30 text-yellow-400 text-[10px] font-extrabold">
                  Mpamba
                </span>

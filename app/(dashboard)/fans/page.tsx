...
export default function FansPage() {
  ...
  useEffect(() => {
    fetch("/api/fans?limit=100", { cache: "no-store" })
      .then(async (response) => {
        const payload = await response.json();
        if (!response.ok || !payload.success) {
          throw new Error(payload.error || "Failed to load fans");
        }
        setFans(payload.data ?? []);
      })
      .catch((err) => setError(err instanceof Error ? err.message : "Failed to load fans"))
      .finally(() => setLoading(false));
  }, []);

  const filtered = useMemo(() => {
    const query = search.trim().toLowerCase();
    if (!query) return fans;
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-400">
            Persistent Instagram audience records built from real webhook interactions.
          </p>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-xs text-zinc-400">
          <span className="font-black text-white">{fans.length}</span> known fan{fans.length === 1 ? "" : "s"}
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[760px] text-left text-xs">
              <thead className="border-b border-white/[0.08] bg-white/[0.02] text-[11px] font-bold uppercase tracking-wider text-zinc-500">
                <tr>
                  <th className="px-5 py-4">Fan</th>
                  <th className="px-4 py-4">Tags</th>
                  <th className="px-4 py-4 text-center">Interactions</th>
                  ...
                  <tr key={fan.id} className="transition-colors hover:bg-white/[0.02]">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 text-xs font-black text-black">
                          {displayName(fan).slice(0, 1).toUpperCase()}
                        </div>
                        <div className="min-w-0">
                          <p className="truncate font-bold text-white">@{displayName(fan)}</p>
                          <p className="truncate font-mono text-[10px] text-zinc-600">{fan.instagramUserId}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex flex-wrap gap-1.5">
                        {fan.tags.length ? (
                          fan.tags.map((tag) => (
                            <span key={tag} className="rounded-md border border-emerald-400/15 bg-emerald-400/5 px-2 py-1 text-[10px] font-bold text-emerald-300">
                              {tag}
                            </span>
                          ))
                        ) : (
                          <span className="text-zinc-600">Unlabelled</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-4 text-center font-mono font-bold text-amber-400">
                      {fan.interactionCount}
                    </td>
                    <td className="px-5 py-4 text-right font-mono text-[11px] text-zinc-500">
                      {formatTime(fan.lastInteractionAt)}
                    </td>
                  </tr>
                ))}
              </tbody>
